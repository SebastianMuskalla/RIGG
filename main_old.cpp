...
{
    WorklistKleene* s = new WorklistKleene(A, G, use_subsumption, max_time);
    s->solve();
    Formula* sol = s->formulaFor(word);

    bool res = sol->isRejecting();//    delete sol;
    bool timeout = s->timeout_flag;

    delete s;

    return make_pair(res, timeout);
}
 ...
 */
tuple<bool, uint, uint, uint, uint, bool> cachatMinWithMeasuring (NFA* A, GameGrammar* G, vector<Letter*> word)
{
    auto start = chrono::steady_clock::now();
 ...
 */
tuple<bool, bool, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint> timeMeasuring (
...
//    bool res_cachat_2 = get<0>(cachat_2);
    ...
    if (!get<5>(cachat_min_1) && !get<5>(cachat_min_2) && (
...
//    uint cachat_total = cachat_determinize + cachat_generate + cachat_saturate;
    ...
    return tuple<bool, bool, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint>(res_worklist_dfa_1,
                                                                                                     res_worklist_dfa_2,
                                                                                                     worklist_time,
                                                                                                     0, 0, 0, 0,
//                                                                                               cachat_total,
        ...
        tuple<NFA*, GameGrammar*, vector<Letter*>, vector<Letter*>> t)
{
    ...
    return tuple<uint, uint, uint, uint, uint, uint>(naive_time, naive_dfa_timeouts, worklist_time,
                                                     worklist_dfa_timeouts, cachat_min_total, cachat_timeouts);
